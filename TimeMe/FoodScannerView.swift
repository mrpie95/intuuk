...
struct FoodScannerView: View {
    ...
    /// protein, carbs, fat, basisGrams
    let onConfirm: (Double, Double, Double, Double) -> Void

    @StateObject private var scanner  = NutritionScanner()
    ...
    var body: some View {
        ZStack {
            ...
            ScannerOverlay(scanner: scanner, isLocked: isLocked, onClose: { dismiss() })
        }
        .preferredColorScheme(.dark)
        .onAppear    { scanner.start() }
        .onDisappear { scanner.stop()  }
        .onChange(of: scanner.isLocked) { _, locked in
            guard locked else { return }
            withAnimation(.spring(duration: 0.4, bounce: 0.25)) { isLocked = true }
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.9) {
                onConfirm(scanner.protein ?? 0, scanner.carbs ?? 0, scanner.fat ?? 0,
                          scanner.basisGrams)
                dismiss()
            }
        }
...
private struct ScannerOverlay: View {
    ...
    private var bracketOpacity: Double {
        isLocked ? 0 : 0.35 + scanner.confidence * 0.65
    }

    var body: some View {
        ZStack {
            RadialGradient(colors: [.clear, .black.opacity(0.58)],
            ...
            VStack(spacing: 0) {
                ...
                // ── Brackets → success ring ───────────────────────
                ZStack {
                    ScanBracketsShape()
                        .stroke(.white.opacity(bracketOpacity),
                                style: StrokeStyle(lineWidth: 1.5, lineCap: .round, lineJoin: .round))
                        .scaleEffect(isLocked ? 0.88 : 1)
                        .opacity(isLocked ? 0 : 1)
                        .animation(.easeOut(duration: 0.28), value: isLocked)
                ...
                .animation(.spring(duration: 0.45, bounce: 0.15), value: isLocked)

                // ── Hint text ─────────────────────────────────────
                Text("Aim at the nutrition label")
                    .font(.system(size: 12))
                    .foregroundStyle(.white.opacity(isLocked ? 0 : 0.28))
                    .padding(.top, 14)
                ...
                // ── Macro cards ───────────────────────────────────
                MacroCardRow(scanner: scanner, isLocked: isLocked)
...
private struct MacroCardRow: View {
    @ObservedObject var scanner: NutritionScanner
    let isLocked: Bool

    private var basisLabel: String? {
        guard scanner.protein != nil else { return nil }
        return "per \(Int(scanner.basisGrams))g"
    }

    var body: some View {
        VStack(spacing: 10) {
            HStack(spacing: 10) {
                MacroCard(label: "Protein", value: scanner.protein,
                          color: .white.opacity(0.9), isLocked: isLocked)
                MacroCard(label: "Carbs",   value: scanner.carbs,
                          color: Color(hex: "FFD7A0"), isLocked: isLocked)
                MacroCard(label: "Fat",     value: scanner.fat,
                          color: Color(hex: "AAF0C8"), isLocked: isLocked)
            }

            // Basis label fades in once we have at least protein
            if let label = basisLabel {
                Text(label)
                    .font(.system(size: 11))
        ...
        .animation(.easeInOut(duration: 0.3), value: basisLabel)
    }
}
    ...
    var body: some View {
        VStack(spacing: 5) {
            ...
            Text(label)
                ...
                .tracking(0.7)
                .textCase(.uppercase)
                .foregroundStyle(hasValue ? color.opacity(0.75) : .white.opacity(0.2))
                .animation(.easeInOut(duration: 0.25), value: hasValue)

            // Value
            Text(value.map { String(format: "%.1f", $0) + "g" } ?? "—")
                .font(.system(size: 38, weight: .light))
                .monospacedDigit()
                .foregroundStyle(hasValue ? color : .white.opacity(0.12))
                .contentTransition(.numericText())
                .animation(.spring(duration: 0.3), value: value)
        }
        .frame(maxWidth: .infinity)
        .padding(.vertical, 18)
        // Dark frosted card — readable over any camera frame
        .background(
            RoundedRectangle(cornerRadius: 18)
                .fill(.black.opacity(hasValue ? 0.45 : 0.22))
        )
        .overlay(
            RoundedRectangle(cornerRadius: 18)
                .stroke(hasValue ? color.opacity(isLocked ? 0.55 : 0.28) : .white.opacity(0.07),
                        lineWidth: 1)
        )
        .scaleEffect(isLocked ? 1.05 : 1)
        .animation(.spring(duration: 0.45, bounce: 0.2), value: isLocked)
        .animation(.easeInOut(duration: 0.3), value: hasValue)
    }
}

// MARK: - Lock success animation (circle → tick)

private struct LockSuccessView: View {
    ...
    var body: some View {
        ...
        .onAppear {
            withAnimation(.easeOut(duration: 0.42))             { ringTrim = 1 }
            withAnimation(.easeOut(duration: 0.32).delay(0.36)) { tickTrim  = 1 }
        }
    }
}
